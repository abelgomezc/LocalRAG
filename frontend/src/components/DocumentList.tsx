...
export function DocumentList({ onDeleted, onSelectDocument }: { onDeleted: () => void; onSelectDocument?: (fileName: string, fileType: string, id: number) => void }) {
    ...
    <div className="document-list">
      ...
      {documents.length === 0 ? (
        ...
        <ul>
          {documents.map((doc) => (
            <li key={doc.id}>
              <div className="doc-name" style={{ cursor: onSelectDocument ? 'pointer' : 'default' }} onClick={() => onSelectDocument?.(doc.fileName, doc.fileType || 'application/pdf', doc.id)}>
